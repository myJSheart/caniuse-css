...
export default {
  ...
  activate(state) {
    ...
    const prefixFamily = this.groupAttributesNameByPrefix(attributeNames);
    console.log(prefixFamily);
    ...
    this.subscriptions.add(editor.onDidChangeCursorPosition((event) => {
      ...
      bottomMessage.innerHTML = this.compatibilityToHTML(css, compatibility, fuzzySet, prefixFamily);
    }));
  ...
  */
  ...
  compatibilityToHTML(CSSAttribute, compatibility, fuzzySet, prefixFamily) {
    let htmlString = '';
    let matchArray = '';
    if (Object.keys(compatibility).length === 0) {
      if (CSSAttribute.trim() !== '') {
        if (prefixFamily[CSSAttribute.trim()] !== null && prefixFamily[CSSAttribute.trim()] !== undefined) {
          return `<div><div class="jr-studio-attribute">${CSSAttribute}</div><div class="jr-studio-not-found">Not Found</div></br><div class="jr-studio-tips"><div style={ color: 'white' }>Do you mean: </div><b>${prefixFamily[CSSAttribute.trim()].toString()}</b></div></div>`;
        }
  ...
  /**
   * split attributes' names by '-' and record the prefix array
   * @param attributesName: input array
   * @param prefixFamily: output array, records the prefix family which has more than one child.
   */
  groupAttributesNameByPrefix(attributesName) {
    let tempFamily = {};
    attributesName.forEach((name) => {
      if (name.split('-').length !== 0) {
        if (!Array.isArray(tempFamily[name.split('-')[0]])) {
          tempFamily[name.split('-')[0]] = [];
        }
        tempFamily[name.split('-')[0]].push(name);
      }
    });
    const prefixFamily = {};
    Object.keys(tempFamily).forEach((prefix) => {
      if (tempFamily[prefix].length > 1) {
        prefixFamily[prefix] = tempFamily[prefix];
      }
    });
    return prefixFamily;
  },
